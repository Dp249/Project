...
def runRoutineOneBack ():
    ...
    for thisBlock1_1back in Block1_1back:
        ...
        key_resp.append (key_resp_3.keys)
    ...
    df = pd.DataFrame ({'time':pd.Timestamp.now(),'stimuli':letters_l,'response time':key_rt,'bpms':bpms, 'key_resp':key_resp, 'fix_started':fix_started,'fix_end':fix_end, 'correctness':correct, 'difficulty':diff})
    df.to_excel ('data/experiment1_no_sound.xlsx')
...
def runRoutineTwoBack ():
        ...
        while continueRoutine and routineTimer.getTime() > 0:
            ...
            if key_resp_3.status == STARTED and not waitOnFlip:
                ...
                if len(_key_resp_3_allKeys):
                    ...
                    continueRoutine = False
            #mySound = sound.Sound('A')
            print (all_letters[iteration], ' ', all_letters[iteration - 1], all_letters[iteration - 2]!= all_letters[iteration])
    ...
    Block2_2back.saveAsText(filename + 'Block2_2back.csv', delim=',',
        ...
        dataOut=['n','all_mean','all_std', 'all_raw'])
    diff = [rating] * len (bpms)
    df = pd.DataFrame ({'stimuli':letters_l,'response time':key_rt,'bpms':bpms, 'key_resp':key_resp, 'fix_started':fix_started,'fix_end':fix_end, 'correctness':correct, 'difficulty':diff})
    df.to_excel ('data/experiment2_no_sound.xlsx')
    # ------Prepare to start Routine "Break"-------
    continueRoutine = True
    # update component parameters for each repeat
    key_resp_4.keys = []
...
def runRoutineThreeBack ():
    ...
    for thisBlock3_3back in Block3_3back:
        ...
        while continueRoutine and routineTimer.getTime() > 0:
            ...
            if 'space' in key_resp_3.keys and flag==False and iteration > 0 and iteration < len (all_letters) and all_letters[iteration] != all_letters[iteration - 3]:
                print (all_letters[iteration], ' ', all_letters[iteration - 1])
                nextFlip = win.getFutureFlipTime(clock='ptb')
                mySound.play(when = nextFlip)  # sync with screen refresh
                print ('CLICK.................................................')
    ...
    diff = [rating] * len (bpms)
    df = pd.DataFrame ({'stimuli':letters_l,'response time':key_rt,'bpms':bpms, 'key_resp':key_resp, 'fix_started':fix_started,'fix_end':fix_end, 'correctness':correct, 'difficulty':diff})
    df.to_excel ('data/experiment3_no_sound.xlsx')
    # get names of stimulus parameters
